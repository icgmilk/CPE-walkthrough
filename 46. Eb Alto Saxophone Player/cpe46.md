...
題目連結: [Eb Alto Saxophone Player](https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=1356)
![46-1](pic/46-1.jpg)

這題要計算演奏薩克斯風每根手指分別按了幾下。
這題麻煩的部分是音調有好幾個，每個都有不同的指法要記。不過整體邏輯不會太複雜。
把每個指法對應的 case 建好後，還要考慮手指是不是持續按著的情況，所以會有 `finger[10]` 跟 `before[10]`。當before為 0 finger為 1 的情況才紀錄。
